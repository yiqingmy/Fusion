...
int main(int argc, char* argv[]) {
	...
	for (int i = 0; i<files_names.size(); i++) {
		...
		Mat r_img_src;
		cv::resize(img_src,r_img_src,Size(img_src.cols/2,img_src.rows/2));
		
		images.push_back(r_img_src);//For the registration.
	}
	//**************Test for the image registration**************
	
	vector<Mat>matches;
	double T1 = (double)cv::getTickCount();
	sc::MatchImg(images,matches,2);

	double T2 = (double)cv::getTickCount();
	cout << "Total time:" << ((double(T2 - T1)) / cv::getTickFrequency()) << endl;
	Mat result;
	//result = GFF(images, "color", 3, 5);
	result = GFF(matches, "color", 3, 5);
	imwrite("C:\\Users\\mao_y\\Desktop\\fused.jpg", result);
	//****************************************************
	//for (int nth = 1; nth < 9;nth++) {
	//	for (int yth = 1; yth < 9;yth++) {
	//		Mat result = GFF(images, "color", nth,yth);
	//		char tag_file_dst[100];
	//		sprintf_s(tag_file_dst, "C:\\Users\\mao_y\\Desktop\\results\\ADoG_O%d_L%d.jpg", nth,yth);
